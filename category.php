...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    $categories = $db->prepare("SELECT * FROM tbl_categories WHERE  clm_id = ? AND clm_status = 'publish' AND clm_place = 'website'");
    $categories->execute(array($id));
    $categorie = $categories->fetch();

   if(!empty($categorie)){ ?>
       <h1 class="title " style="font-weight: 700;"><?php echo $categorie["clm_name"] ?></h1>
       <div class="c-seo-copy" style="text-align: center;margin: 10px 0px;"><?php echo $categorie["clm_description"] ?></div>
      <?php }

        $header_offerss = $db->prepare("SELECT * FROM tbl_header_offers {$query} {$sort}");
		$header_offerss->execute(array($id));
                ...
                <div class="scroller scroller-hero highlight-ads">
                    <div class="">
                        <div class="highlight-scroll heroSlides" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
                                            ...
                                            <div class="highlight-content">
                                                <div class="highlight-branding">
                                                    <div class="logo-container partner-logo">
                                                        <a href="<?php if($header_offer["clm_type"] == "link"){ echo $header_offer["clm_link"]; }else{  echo "offer.php?id={$header_offer["clm_id"]}&o={$header_offer["tbl_name"]}&c={$header_offer["clm_parent"]}"; } ?>" aria-label="<?php echo $header_offer["clm_name"] ; ?>">
                                                ...
                                                <a class="call-to-action" style="position:relative;z-index: 100;" href="<?php if($header_offer["clm_type"] == "link"){ echo $header_offer["clm_link"]; }else{  echo "offer.php?id={$header_offer["clm_id"]}&o={$header_offer["tbl_name"]}&c={$header_offer["clm_parent"]}"; } ?>" 
                                                   aria-label="<?php echo $header_offer["clm_name"]; ?>">Get now</a>
                                            </div>
                                        </div>
                            ...
                            <button type="button" class="showcase__scroll-prev ud-icon" style="position:relative;z-index: 10;" title="Previous"><i class="fas fa-arrow-left"></i></button>
        ...
        ?>
	     ...
	     <?php
			...
			$main_offerss = $db->prepare("SELECT * FROM tbl_main_offers  {$query} {$sort} LIMIT 6");
			$main_offerss->execute(array($id));
        ...
        ?>
                ...
                <div class="scroller" >
                    ...
                    <h2 class="title pull-left">
                        Boosted Offers!
                        <a href="main.php?id=<?php echo $id ; ?>" title="View All" class="pull-right">View all <i class="fas fa-arrow-right"></i></a>
                    </h2>
...
<?php
            ...
            $small_offerss = $db->prepare("SELECT * FROM tbl_small_offers {$query} {$sort} LIMIT 6");
			$small_offerss->execute(array($id));
        ...
        <div class="scroller" >
               ...
               <h2 class="title pull-left">
                        Boosted Offers!
                        <a href="small.php?id=<?php echo $id ; ?>" title="View All" class="pull-right">View all <i class="fas fa-arrow-right"></i></a>
                    </h2>
            ...
            <div class="tile__scroll-onebyone">
                ...
                <?php foreach ($small_offers as $small_offer) { ?>
                    <article class="tile tile-onebyone" >
                        <a href="<?php if($small_offer["clm_type"] == "link"){ echo $small_offer["clm_type"]; }else{ echo "offer.php?id={$id}&o={$o}&c={$c}"; } ?>" 
                        aria-label="<?php echo $small_offer["clm_name"]; ?>">
                            <div class="tile__inner">
        ...
        <?php
			...
			$linkss = $db->prepare("SELECT * FROM tbl_links WHERE  clm_status = 'publish'  AND clm_parent = ? LIMIT 2");
        ...
        <div class="scroller">
                ...
                <h2 class="title pull-left">
                        Boosted Offers!
                        <a href="link.php?id=<?php echo $id ; ?>" title="View All" class="pull-right">View all <i class="fas fa-arrow-right"></i></a>
                    </h2>
